...
<html> <!-- 모든 HTML 페이지의 기본 요소로, 모든 태그는 이 html태그 내부에 작성함 -->
    <head><!-- Body 태그에 필요한 스타일시트와 자바스크립트를 제공함 -->
...
검사 기능은 다양하게 있으며 또는 웹 페이지를 대상으로 검사를 실행할 수 있음
...
## 글자 모양 태그
HTML5는 글자 모양 태그를 사용해 웹 페이지의 글자에 형태와 의미를 부여함
태그 | 설명
:--:|:--:
b | 굵은 글자<sub>bold
i | 기울어진 글자<sub>italic
small| 작은 글자
sub| 아래 첨자<sub>subscript
sup| 위 첨자<sub>superscript
ins| 밑줄 글자<sub>insert
del| 취소선이 그어진 글자<sub>delete
+ small 태그는 있지만 HTML5에는 big태그가 없음<sub>HTML4에는 있었음
+ 글자 모양 태그는 단독으로 사용이 가능
+ 제목 글자와 본문 글자 태그 내부에 입력 또는 다른 글자 모양 태그도 넣을 수 있음
#### 주의
