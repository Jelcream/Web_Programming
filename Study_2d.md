...
+ 특허권으로 보호되지 않는 오픈 표준 파일 형식으로 멀티미디어 비트스트림을 효율적으로 전송하고 처리할 수 있게 하기 위해 Xiph.Org 재단에서 개발
...
<br><br>
## 기본 입력 양식 태그
+ input 태그에 type 속성을 지정해서 다양한 종류의 기본 입력 양식을 생성
+ value 속성을 이용해서 내부 값을 지정할 수 있음

### name 속성
+ 라디오 버튼의 name속성을 사용하면 여러 대상 중 하나만 선택하는 형태를 구현
+ name을 동일하게 만들어줌으로 하나의 그룹으로 묶이게 됨

라디오 버튼이나 checkbox 버튼은 기본적으로 초기값을 지정하는 것이 가능하다. -> checked 속성 (속성값을 가지지 않는다.)

<br><br>

## Select 태그
+ 목록으로 보여 주는 항목 중 하나 또는 여러 개를 선택할 때 사용하는 입력 양식 요소
+ 기본적으로 하나만 선택이 가능
+ optgroup 태그와 option태그와 함께 사용
+ 여러개를 선택하기 위해서는 `multiple`속성의 `"multiple"` 값을 지정 -> 에제 파일 참조
+ 옵션 선택 태그는 자바스크립트를 사용해 하나하나 만드는 것이 일반적
<br><br>
...
## div 태그 와 span 태그
+ 공간 분할 태그
+ div 태그는 공간을 블록<sub>block</sub> 형식으로 분할
+ span 태그는 공간을 인라인<sub>inline</sub> 형식으로 분할
+ h1 태그와 p 태그는 블록 형식
+ a 태그와 i 태그는 인라인 형식

## 시맨틱 태그
시맨틱은 '의미론적인'이라는 의미, 특정 태그에 의미를 부여해 웹 페이지를 만들기 시작했는데, 이름 시맨틱 웹이라고 표현
+ 시맨틱 웹은 컴퓨터 프로그램이 코드를 읽고 의미를 인식할 수 있는 지능형 웹
+ div태그로도 웹 페이지의 레이아웃을 구성할 수 있음
+ 시맨틱 태그를 사용해 각 태그에 의미를 부여하는 것이 더 가독성이 좋음
  태그|설명
  :--:|:--
